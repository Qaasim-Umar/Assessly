...
export default function NyscForm({ initial, mode }: { initial?: NyscData; mode: "new" | "edit" }) {
    const router = useRouter();
    const [form, setForm] = useState<NyscData>(initial ?? {
        slug: "", title: "", batch_label: "", content: "", published: false,
    });
    const [slugLocked, setSlugLocked] = useState(!!initial?.slug);
    const [saving, setSaving] = useState(false);
    const [error, setError] = useState("");

    function set<K extends keyof NyscData>(k: K, v: NyscData[K]) {
        setForm(p => ({ ...p, [k]: v }));
    }
    ...
    async function save(published: boolean) {
        ...
        try {
            if (mode === "edit" && form.id) {
                const { error: e } = await supabase.from("admissions_nysc").update(payload).eq("id", form.id);
                if (e) throw e;
            } else {
                const { error: e } = await supabase.from("admissions_nysc").insert(payload);
                if (e) throw e;
            }
            router.push("/general/dashboard/admissions");
        } catch (e: unknown) {
            const msg = e instanceof Error ? e.message : String(e);
            setError(msg.includes("duplicate") ? "An NYSC entry with this slug already exists." : "Failed to save.");
        } finally {
            setSaving(false);
        }
    }
        ...
        <div className="min-h-screen bg-[#f0f2f5]">
            <header className="bg-white border-b border-gray-200 sticky top-0 z-20 shadow-sm">
                <div className="max-w-3xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-gray-700">{mode === "edit" ? "Edit NYSC Post" : "New NYSC Post"}</span>
                    </div>
            ...
            <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-5">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                    <Label>Title</Label>
                    <input type="text" value={form.title} onChange={e => handleTitle(e.target.value)} placeholder="e.g. 2026 Batch A Stream II Mobilization Opens" className="w-full text-xl font-bold text-gray-900 placeholder:text-gray-300 border-0 outline-none mt-1" />
                </div>

                <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                    <Label>Batch</Label>
                    <input type="text" value={form.batch_label} onChange={e => set("batch_label", e.target.value)} placeholder="2026 Batch A Stream II" className="w-full mt-1 text-sm text-gray-700 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-green-500" />
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                    ...
                    <textarea value={form.content} onChange={e => set("content", e.target.value)} placeholder="Write the NYSC update here…" rows={12} className="w-full text-sm text-gray-700 placeholder:text-gray-300 border-0 outline-none resize-y mt-1" />
                </div>

                <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                    <Label>Slug</Label>
                    <div className="flex items-center gap-2 mt-1">
                        <input type="text" value={form.slug} onChange={e => { set("slug", e.target.value); setSlugLocked(true); }} placeholder="2026-batch-a-stream-ii-mobilization" className="flex-1 text-sm text-gray-700 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-green-500 font-mono" />
