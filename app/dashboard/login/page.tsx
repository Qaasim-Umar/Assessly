            ...
            <div className="flex-1 flex flex-col items-center justify-center px-4 py-12">
                <div className="w-full max-w-sm bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden z-10 relative">
                    ...
                    <div className="bg-gray-900 px-8 py-7 text-center">
                        ...
                        <h1 className="text-white text-xl font-bold tracking-tight">Assessly Admin</h1>
                        <p className="text-gray-400 text-xs mt-1">Teacher & Administrator Portal</p>
                    </div>
                    ...
                    <div className="flex border-b border-gray-100">
                        {(["login", "signup"] as Tab[]).map((t) => (
                            <button key={t} type="button" onClick={() => switchTab(t)}
                                className={`flex-1 py-3 text-sm font-semibold transition-colors relative ${tab === t ? "text-gray-900 bg-gray-50" : "text-gray-400 hover:text-gray-600 bg-white"}`}>
                                {t === "login" ? "Sign In" : "Create Account"}
                                {tab === t && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gray-900" />}
                            </button>
                        ))}
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="px-8 py-6 space-y-4">
                        {error && (
                            <div className="flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 text-xs px-3 py-2.5 rounded-lg mb-2">
                        ...
                        {success && (
                            <div className="flex items-start gap-2 bg-green-50 border border-green-200 text-green-700 text-xs px-3 py-2.5 rounded-lg mb-2">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} required placeholder="Admin username" className={inputBase} />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                ...
                                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required placeholder={tab === "login" ? "Enter password" : "Create password (min 6 chars)"} className={`${inputBase} ${tab === "signup" ? "" : "pr-11"}`} minLength={6} />
                                {tab === "login" && (
                        ...
                        {tab === "signup" && (
                            <div className="space-y-1.5">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Re-enter password" required className={inputBase} />
                                </div>
                            </div>
                        )}

                        <button type="submit" disabled={loading}
                            className="w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-black disabled:opacity-60 text-white font-bold text-sm py-3 rounded-lg transition-colors mt-2">
                            {loading
                        ...
                        <p className="text-center text-xs text-gray-400 pt-1">
                            Student?{" "}
                            <a href="/login" className="text-gray-700 hover:underline font-semibold">Take an exam →</a>
                        </p>
                    </form>
                </div>
            </div>
