...
export default function ProcessingStep({ subject, questionCount, onComplete }: Props) {
    ...
    return (
        <div className="flex flex-col items-center justify-center py-10 px-6">
            {/* AI Brain Icon */}
            <div className="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center mb-6 relative">
            ...
            <div className="w-full max-w-sm space-y-3">
                {STEPS.map((step, i) => {
                    ...
                    return (
                        <div key={i} className={`flex items-start gap-3 p-3 rounded-lg border transition-all ${isComplete ? "bg-green-50 border-green-200" : isActive ? "bg-green-50 border-green-200" : "bg-white border-gray-100"
                            }`}>
                            <div className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center mt-0.5 ${isComplete ? "bg-green-500" : isActive ? "bg-green-600" : "bg-gray-200"
                                }`}>
                                {isComplete ? (
                            ...
                            <div className="flex-1 min-w-0">
                                <p className={`text-xs font-semibold ${isComplete ? "text-green-700" : isActive ? "text-green-700" : "text-gray-400"}`}>{step.label}</p>
                                {isActive && <p className="text-[10px] text-green-500 mt-0.5">{step.detail}</p>}
